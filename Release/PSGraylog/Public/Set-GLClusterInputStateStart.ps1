...
.NOTES
    Auto generated
#>
function Set-GLClusterInputStateStart {
    ...
    process {
        if ($PSCmdlet.ShouldProcess($Inputid,"Start or restart specified input in all nodes")) {
            $BodyHash = @{}
            ...
            try { Invoke-RestMethod -Method PUT -Headers $Headers -ContentType 'application/json' -Uri "$APIUrl$APIPath" -Credential $Credential -Body $Body -ErrorAction Stop } catch { if ($Error[0].Exception.Response.StatusCode.value__ -eq 404) { Write-Error -Exception $Error[0].Exception -Message "No such input." -ErrorAction $ErrorActionPreference } else { Write-Error -Exception $Error[0].Exception -Message $Error[0].Message -ErrorAction $ErrorActionPreference } }
