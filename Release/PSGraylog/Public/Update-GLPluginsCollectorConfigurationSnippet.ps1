...
.DESCRIPTION
    ...
    For more information about this cmdlet - please search for /plugins/org.graylog.plugins.collector/configurations/{id}/snippets/{snippet_id} in the docs available at http(s)://your-graylog.fqdn/api/api-docs
.EXAMPLE
    Update-GLPluginsCollectorConfigurationSnippet -Id <String> -Snippet_Id <String> -Body <PSCustomObject>
.NOTES
    Auto generated
#>
function Update-GLPluginsCollectorConfigurationSnippet {
    ...
    process {
        if ($PSCmdlet.ShouldProcess($Id,"Update a configuration snippet")) {
            $QueryArray = @()
            ...
            if (![string]::IsNullOrEmpty($Snippet_Id)) {
                $Snippet_Id = [system.web.httputility]::UrlEncode($Snippet_Id)

                $QueryArray += "snippet_id=$Snippet_Id"
            }
            ...
            $APIPath = '/plugins/org.graylog.plugins.collector/configurations/{id}/snippets/{snippet_id}'
